...
public class Heuristic {
	...
	public ArrayList<Tuple> getCandidateServers(Graph G, int minDemand, int[][] capacity){
	...
	public OverlayMapping executeHeuristic(Graph G, int[][] M, Flow f, ArrayList<Tuple> E, int[] mbSpecs){
		...
		ArrayList<Tuple> candidateNodes = getCandidateServers(G, minDemand, capacity);//new ArrayList<Tuple>();
		...
		for(int i=0;i<validChains.size();i++){
			...
			OverlayMapping om = getOverlaySolution(G, validChains.get(i), mbSpecs,candidateNodes, capacity);
			if(om != null)
				omSol.add(om);
		}
		...
		/*for(int i=0; i<omSol.size(); i++){
			...
			for(int j=0;j<omSol.get(i).getChainOrder().size()+2;j++){
				System.out.println(j+"-"+omSol.get(i).getChainOrder().size());
				Dijkstra djst = new Dijkstra(G,capacity);
				Tuple tup;
				int sourceIndex;
				int destinationIndex;
				//Route from Ingress to Starting Node
				if(j == omSol.get(i).getChainOrder().size()){
					System.out.println("Is Ingress Path");
					sourceIndex = f.getSource();
					destinationIndex = omSol.get(i).getChainOrder().get(0).getSource();
					tup = new Tuple(sourceIndex,destinationIndex);						
				}else{
				if(j == omSol.get(i).getChainOrder().size()+1){
					sourceIndex = omSol.get(i).getChainOrder().get(omSol.get(i).getChainOrder().size()-1).getDestination();
					destinationIndex = f.getDestination();
					tup = new Tuple(sourceIndex,destinationIndex);							
				}
				else{
					tup = omSol.get(i).getChainOrder().get(j);
					sourceIndex = omSol.get(i).getNodeMapping(getIndexNF(f, tup.getSource()));
					destinationIndex = omSol.get(i).getNodeMapping(getIndexNF(f,tup.getDestination()));
				}}
				if(omSol.get(i).linkMapping.containsKey(tup))
					continue;
				else{
					ArrayList<Tuple> path = djst.getPath(sourceIndex, destinationIndex, f.getBw());
					if(path == null || path.size() == 0){
						omCosts.add(i,Integer.MAX_VALUE);
						break;
					}
					else{
						capacity = updateNodeCap(path, capacity, f.getBw(), false); // Increment Capacity Matrix
			  ...
			  Iterator it = omSol.get(i).linkMapping.entrySet().iterator();
			    while (it.hasNext()) {
			        Map.Entry pair = (Map.Entry)it.next();
					//if(omSol.get(i).linkMapping.containsKey(pair.getKey()))
						capacity = updateNodeCap((ArrayList<Tuple>)pair.getValue(), capacity, f.getBw(), true);	
			    }
			}*/
	...
	public OverlayMapping getOverlaySolution(Graph G, ArrayList<Tuple> chain, int [] mbSpecs, ArrayList<Tuple> candidateNodes, int [][] capacity){
	...
	public OverlayMapping performNodeEmbedding(Graph G, ArrayList<Tuple> chain, int [] mbSpecs, ArrayList<Tuple> candidateNodes){
		...
		for(int j=0;j<candidateNodes.size();j++){
			...
			for(int t=0;t<=chain.size();t++){ //For every NF in the chain
				...
				for(int k=t;k<chain.size();k++){ //Loop through the remaining tuples
	...
	public OverlayMapping performLinkEmbedding(OverlayMapping omSol, Graph G, int[][] capacity){
		...
		for(int j=0;j<omSol.getChainOrder().size()+2;j++){
			...
			if(j == omSol.getChainOrder().size()){
				System.out.println("Routing Ingress Path...."+"Starting NF Type ="+omSol.getChainOrder().get(0).getSource());
				sourceIndex = f.getSource();
				destinationIndex = omSol.getNodeMapping(getIndexNF(f,omSol.getChainOrder().get(0).getSource()));
				tup = new Tuple(sourceIndex,destinationIndex);						
			}else{
			if(j == omSol.getChainOrder().size()+1){
				System.out.println("Routing Egress Path...."+" Last NF Type ="+omSol.getChainOrder().get(omSol.getChainOrder().size()-1).getDestination());
				sourceIndex = omSol.getNodeMapping(getIndexNF(f, omSol.getChainOrder().get(omSol.getChainOrder().size()-1).getDestination()));
				destinationIndex = f.getDestination();
				tup = new Tuple(sourceIndex,destinationIndex);							
			}
			else{
				...
				destinationIndex = omSol.getNodeMapping(getIndexNF(f,tup.getDestination()));
			}}
			if(omSol.linkMapping.containsKey(tup))
				continue;
			else{
				ArrayList<Tuple> path = djst.getPath(sourceIndex, destinationIndex, f.getBw());
				if((path == null)||(path.isEmpty() && sourceIndex != destinationIndex)){
